...
"""
Flags: The tf.app.flags module is presently a thin wrapper around argparse,
...
"""
///DESC///  --- Converts data from binary format to text format
...
def _binary_to_text():
  #open the input file supplied by commmand line as argument of 'in_file'
  reader = open(FLAGS.in_file, 'rb')

  #open the input file supplied by commmand line as argument of 'out_file'
  ...
  while True:
    ...
    if not len_bytes:
      ...
      return
    str_len = struct.unpack('q', len_bytes)[0]#The result of --struct.unpack-- is a tuple, hence [0] is used
